...
import { sectionsToHTML, sectionsToHTMLWithoutImages } from '../components/TermsCondition';
...
useEffect(() => {
  if (customer) {
    setQuotationData(prev => ({
      ...
      customer: customer.name || "",
      contact: customer.phone || "",
      // ✅ Auto-fill TRN from customer
      trn: customer.trn || customer.taxRegistrationNumber || "",
    }));
  }
}, [customer]);
  ...
  : '';
    ...
    try {
      ...
      // Add this function inside QuotationTemplateInner component
const extractTermsImagesFromSections = useCallback((sections) => {
  ...
  sections.forEach((section, sectionIndex) => {
    if (section.images && Array.isArray(section.images) && section.images.length > 0) {
      section.images.forEach((img, imgIndex) => {
        // Only send images that are base64 (not yet uploaded)
        // Check if the URL is a base64 string (starts with data:)
 ...
 const handleSubmit = useCallback(async () => {
  ...
  try {
    ...
    // ✅ Extract terms images from tcSections (base64 images that need Cloudinary upload)
    ...
    const finalTrn = quotationData.trn?.trim() || customer?.trn || customer?.taxRegistrationNumber || "";
    ...
    quotationItems, itemImages, uploadedDocuments, tcSections, extractTermsImagesFromSections, addQuotation, user, navigate]);
