...
import {
  Users, FileText, TrendingUp, AlertCircle, RefreshCw,
  Search, X, CheckCircle, Loader, LogOut, Plus, Calendar,Clock, Award,Ban,Eye,Download,Trash2
} from 'lucide-react';
...
import { 
  StatusBadge, RejectionNote, Toast, StatCard, ActionBtn,
  SortHeader, PaginationBar, SkeletonRow, ConfirmModal, AwardModal 
} from '../components/SharedComponents';

import { 
  PAGE_SIZE_OPTIONS, DEBOUNCE_MS, TAB_KEYS, DELETABLE, STATUS_CONFIG 
} from '../utils/constants';
import { fmtCurrency, fmtDate, isExpired, isExpiringSoon } from '../utils/formatters';
import { downloadQuotationPDF, printQuotation } from '../utils/pdfGenerator';
...
export default function HomeScreen({ onNavigate, onViewQuotation }) {
  ...
  return (
    <div style={{ minHeight:'100vh', backgroundColor:'#f1f5f9', fontFamily:"'Segoe UI', system-ui, sans-serif" }}>
      ...
      >
        {deleteModal.quotation?.status === 'ops_rejected' && (
          <div style={{ 
            ...
            padding:'0.6rem 0.875rem', marginBottom:'0.75rem', fontSize:'0.8rem',
      ...
      {/* Sticky Topbar */}
      <div style={{ 
        backgroundColor:'#0f172a', padding:'0 2rem', display:'flex', 
        alignItems:'center', justifyContent:'space-between', height:60,
        position:'sticky', top:0, zIndex:50, boxShadow:'0 2px 8px rgba(0,0,0,0.25)' 
      }}>
        <div>
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'2rem' }}>
        ...
        {loadError && (
          <div style={{ 
            ...
            padding:'0.875rem 1rem', marginBottom:'1.25rem', fontSize:'0.875rem',
            color:'#991b1b' 
          }}>
            ...
            <div style={{ display:'flex', gap:'0.5rem', alignItems:'center' }}>
              <button onClick={() => clearError()} style={{ background:'none', border:'none', cursor:'pointer', color:'#991b1b', padding:0 }}>
                <X size={14}/>
              </button>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'1rem', marginBottom:'1rem' }}>
          <StatCard 
            ...
            accent="#6366f1" iconBg="#eff1ff" iconColor="#6366f1" 
            Icon={TrendingUp} loading={isInitialLoading} 
            sub={`All quotations combined in ${selectedCurrency}`}
          />
          <StatCard 
            label="Quotations"      
            value={safeQ.length}              
            accent="#8b5cf6" iconBg="#f5f3ff" iconColor="#8b5cf6" 
            Icon={FileText} loading={isInitialLoading} 
            sub="Total submitted"
          />
          <StatCard 
            label="Customers"       
        ...
        {/* Table card */}
        <div style={{ 
          backgroundColor:'#fff', borderRadius:14, boxShadow:'0 1px 4px rgba(0,0,0,0.07)',
          overflow:'visible', position:'relative' 
        }}>

          {/* Card header */}
          <div style={{ 
            padding:'1.125rem 1.5rem', borderBottom:'1px solid #f1f5f9',
            display:'flex', alignItems:'center', justifyContent:'space-between',
            flexWrap:'wrap', gap:'0.75rem' 
          }}>
            <div style={{ 
              display:'flex', gap:'0.2rem', padding:'0.35rem',
              backgroundColor:'#f1f5f9', borderRadius:10, flexWrap:'wrap' 
            }}>
                ...
                return (
                  <button key={key} className="hs-tab" onClick={() => handleTabChange(key)}
                    style={{ 
                      ...
                      transition:'all 0.15s', backgroundColor: active ? '#fff' : 'transparent',
            ...
            <div style={{ display:'flex', alignItems:'center', gap:'0.75rem' }}>
              <button 
                onClick={handleRefresh} disabled={loading} title="Refresh data from server"
                style={{ 
              ...
              >
                <RefreshCw size={14} color="#64748b" 
                  style={loading ? { animation:'hs-spin 1s linear infinite' } : {}}/>
              </button>
              ...
              <div style={{ 
                ...
                <input ref={searchRef}
                  style={{ 
                    ...
                    fontSize:'0.875rem', color:'#0f172a', width:210, fontFamily:'inherit' 
                  }}
          ...
          {isRefreshing && (
            <div style={{ 
              ...
              borderRadius:14, backdropFilter:'blur(1px)' 
            }}>
              <div style={{ 
                display:'flex', flexDirection:'column', alignItems:'center', gap:'0.75rem',
                background:'white', padding:'1.25rem 2rem', borderRadius:12,
                boxShadow:'0 4px 24px rgba(15,23,42,0.12)', border:'1px solid #e2e8f0' 
              }}>
              ...
              <table style={{ width:'100%', borderCollapse:'collapse' }}>
                <thead>
                  <tr style={{ backgroundColor:'#fafafa' }}>
                    {['Quote #','Customer','Project Name','Query Date','Submitted','Expiry','Total','Created By','Actions'].map(h => (
                      <th key={h} style={{ 
                  ...
                  <table style={{ width:'100%', borderCollapse:'collapse' }}>
                    ...
                    <tbody>
                      {paginated.length === 0 ? (
                        <tr>
                          <td colSpan={9} style={{ 
                            padding:'3rem', textAlign:'center', color:'#94a3b8', fontSize:'0.875rem' 
                          }}>
...
// Helper NavBtn component (kept local as it's only used here)
