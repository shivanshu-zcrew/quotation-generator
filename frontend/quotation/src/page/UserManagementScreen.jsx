...
function Field({ label, field, type = 'text', placeholder, formData, errors, onChange, loading, children }) {
  return (
    <div style={formStyles.fieldWrapper}>
      ...
      {children || (
        <input
          ...
          style={{ ...formStyles.input, ...(errors[field] ? formStyles.inputError : {}) }}
          disabled={loading}
        />
      )}
      {errors[field] && <span style={formStyles.errorMsg}>{errors[field]}</span>}
    </div>
  );
}
    ...
    <div style={formStyles.overlay}>
      <div style={formStyles.modal}>
        ...
        <div style={formStyles.body}>
          {/* Name */}
          <Field label="Full Name" field="name" placeholder="e.g. John Smith" formData={formData} errors={errors} onChange={handleChange} loading={loading} />

          {/* Email */}
          <Field label="Email Address" field="email" type="email" placeholder="e.g. john@company.com" formData={formData} errors={errors} onChange={handleChange} loading={loading} />
            ...
            <div style={{ ...formStyles.fieldWrapper, flex: 1 }}>
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ ...formStyles.input, paddingRight: '2.5rem', ...(errors.password ? formStyles.inputError : {}) }}
                  disabled={loading}
                />
                <button
          ...
          <div style={formStyles.fieldWrapper}>
              ...
              {['user', 'admin'].map(r => (
                <button
                  ...
                  style={{
                    ...
                    ...(formData.role === r ? formStyles.roleToggleBtnActive : {}),
                  }}
                >
                  {r === 'user' ? (
            ...
            <p style={formStyles.roleHint}>
              {formData.role === 'admin'
                ? '⚠️ Admins have full access to all features and user management.'
                : 'Standard users can create and manage their own quotations.'}
            </p>
          </div>
        </div>
        ...
        <div style={formStyles.footer}>
          <button onClick={onCancel} style={formStyles.cancelBtn} disabled={loading}>
            Cancel
          </button>
          <button onClick={handleSubmit} style={formStyles.submitBtn} disabled={loading}>
            {loading ? (
              <>
                <span style={formStyles.btnSpinner} />
                Creating...
              </>
            ) : (
              <>
...
export default function UserManagementScreen({ onBack }) {
  ...
  const handleAddUserSuccess = async (message) => {
    ...
    <div style={styles.container}>
      ...
      <div style={styles.statsGrid}>
        {[
          ...
          { label: 'Active Users',  value: stats.active,   color: '#10b981', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
          ...
          { label: 'Admins',        value: stats.admins,   color: '#f59e0b', icon: 'M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
          ...
          <tbody>
            {getFilteredUsers().map(user => (
              <tr key={user._id} style={styles.tr}>
                ...
                <td style={styles.td}>
                  <span style={{ ...styles.roleBadge, backgroundColor: user.role === 'admin' ? '#667eea20' : '#10b98120', color: user.role === 'admin' ? '#667eea' : '#10b981' }}>
                    {user.role}
                  </span>
                </td>
                <td style={styles.td}>
                  <span style={{ ...styles.statusBadge, backgroundColor: user.isActive ? '#10b98120' : '#ef444420', color: user.isActive ? '#10b981' : '#ef4444' }}>
                ...
                <td style={styles.td}>
                  <div style={styles.actionButtons}>
                    <button onClick={() => openRoleModal(user)} style={styles.roleButton} title="Change Role" disabled={actionLoading}>
                    ...
                    <button
                      ...
                      style={{ ...styles.statusButton, backgroundColor: user.isActive ? '#ef444420' : '#10b98120', color: user.isActive ? '#ef4444' : '#10b981' }}
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <div style={styles.modalContent}>
              ...
              <select value={newRole} onChange={e => setNewRole(e.target.value)} style={styles.modalSelect} disabled={actionLoading}>
                <option value="user">User</option>
                <option value="admin">Admin</option>
              </select>
            </div>
            <div style={styles.modalButtons}>
              <button onClick={() => { setShowRoleModal(false); setSelectedUser(null); setNewRole(''); }} style={styles.modalCancelButton} disabled={actionLoading}>
                Cancel
              </button>
              <button onClick={handleRoleChange} style={styles.modalConfirmButton} disabled={actionLoading || newRole === selectedUser.role}>
...
const styles = {
  ...
  statIconContainer: { width: '40px', height: '40px', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  statIcon: { width: '20px', height: '20px' },
  statContent: { flex: 1 },
  statLabel: { fontSize: '12px', color: '#666', marginBottom: '4px' },
  statValue: { fontSize: '20px', fontWeight: 'bold', color: '#1a1a1a' },
  ...
  userAvatar: { width: '32px', height: '32px', borderRadius: '50%', backgroundColor: '#667eea', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', fontWeight: '600' },
  userName: { fontWeight: '500', color: '#1a1a1a' },
  roleBadge: { padding: '4px 8px', borderRadius: '20px', fontSize: '12px', fontWeight: '500' },
  ...
  modal: { backgroundColor: 'white', borderRadius: '12px', padding: '24px', width: '90%', maxWidth: '400px' },
  ...
  modalLabel: { display: 'block', fontSize: '14px', fontWeight: '500', color: '#374151', marginBottom: '8px' },
  ...
  modalConfirmButton: { padding: '8px 16px', backgroundColor: '#667eea', color: 'white', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: '500', cursor: 'pointer' },
...
const formStyles = {
  ...
  roleToggleGroup: { display: 'flex', gap: '10px' },
  roleToggleBtn: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px 14px', border: '1.5px solid #e5e7eb', borderRadius: '8px', fontSize: '14px', fontWeight: '500', color: '#6b7280', backgroundColor: 'white', cursor: 'pointer', transition: 'all 0.15s' },
  roleToggleBtnActive: { borderColor: '#667eea', backgroundColor: '#eff0fe', color: '#667eea', fontWeight: '600' },
  roleHint: { fontSize: '12px', color: '#6b7280', margin: '4px 0 0' },
