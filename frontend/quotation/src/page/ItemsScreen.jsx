...
export default function ItemsScreen({ items, onAddItem, onUpdateItem, onDeleteItem, onBack }) {
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      ...
      setImagePreview(item.imagePath ? `http://51.20.109.158:5000${item.imagePath}` : null);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name.trim() || !formData.price) { alert('Name and price are required'); return; }
    ...
    try {
      const success = editingId ? await onUpdateItem(editingId, fd) : await onAddItem(fd);
      if (success) { closeModal(); alert(editingId ? 'Item updated successfully' : 'Item added successfully'); }
    } catch { alert('Error saving item. Please try again.'); }
  };

  const filtered = items
    .filter(i => i.name.toLowerCase().includes(searchTerm.toLowerCase()) || i.description?.toLowerCase().includes(searchTerm.toLowerCase()))
    .sort((a, b) => {
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4ff', fontFamily: "'Plus Jakarta Sans', 'Inter', system-ui, sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .is-card-btn:hover { transform: translateY(-1px); filter: brightness(.93); }
        ...
        .is-primary-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(99,102,241,.5); }
        ...
        .is-submit-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(99,102,241,.45); }
        .is-cancel-btn {
        ...
        .is-cancel-btn:hover { background: #e2e8f0; }
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div className="fa1" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '.75rem' }}>
            <button className="is-primary-btn" onClick={() => handleOpenModal()}>
              ...
              {filtered.map((item) => (
                <div key={item._id} className="is-card">
                  ...
                  <div className="is-img-area">
                    {item.imagePath ? (
                      <img
                        src={`http://51.20.109.158:5000${item.imagePath}`}
                        alt={item.name}
                  ...
                  <div style={{ padding: '1.1rem 1.1rem .9rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', gap: '.5rem', paddingTop: '.75rem', borderTop: '1px solid #f1f5f9' }}>
                      <button className="is-card-btn edit" onClick={() => handleOpenModal(item)}>
                        <Edit2 size={14} /> Edit
                      </button>
                      <button className="is-card-btn del" onClick={() => { if (window.confirm(`Delete "${item.name}"?`)) onDeleteItem(item._id); }}>
      ...
      {/* ── Modal ── */}
      {showModal && (
        <div className="is-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className="is-modal">
            ...
            <div style={{ padding: '1.5rem 1.5rem 0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <button onClick={closeModal} style={{ background: '#f1f5f9', border: 'none', cursor: 'pointer', borderRadius: '10px', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b', flexShrink: 0, transition: 'background .15s' }}
            ...
            <form onSubmit={handleSubmit} style={{ padding: '1.25rem 1.5rem 1.5rem' }}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input className="is-field" type="text" name="name" placeholder="e.g. Industrial Bearing 6205" value={formData.name} onChange={handleChange} required />
              </div>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input className="is-field" type="number" name="price" placeholder="0.00" value={formData.price} onChange={handleChange} required min="0" step="0.01" />
              </div>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <textarea className="is-field" name="description" placeholder="Describe the item, specs, usage…" value={formData.description} onChange={handleChange} rows={3} style={{ resize: 'vertical', minHeight: '80px' }} />
              </div>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                ...
                <input type="file" accept="image/*" id="is-img-input" style={{ display: 'none' }} onChange={(e) => handleImageChange(e.target.files[0])} />
                ...
                {imagePreview ? (
                  <div style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', border: '1.5px solid #e2e8f0' }}>
                    ...
                    <button
                      ...
                      style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(239,68,68,.9)', color: 'white', border: 'none', borderRadius: '8px', width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
                    >
                      <X size={14} />
                    </button>
                    <label htmlFor="is-img-input" style={{ position: 'absolute', bottom: 8, right: 8, background: 'rgba(255,255,255,.9)', color: '#6366f1', border: '1.5px solid #e0e3ff', borderRadius: '8px', padding: '.3rem .7rem', fontSize: '.75rem', fontWeight: '600', cursor: 'pointer' }}>
                      Change
                    </label>
                  </div>
                ) : (
              ...
              <div style={{ display: 'flex', gap: '.75rem', justifyContent: 'flex-end', paddingTop: '1rem', borderTop: '1px solid #f1f5f9' }}>
                <button type="button" className="is-cancel-btn" onClick={closeModal}>
                  <X size={16} /> Cancel
                </button>
                <button type="submit" className="is-submit-btn">
