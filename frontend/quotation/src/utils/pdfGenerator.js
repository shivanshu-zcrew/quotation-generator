...
export const buildPDFHTML = async (quotation, options = {}) => {
  ...
  const termsImage = quotation.termsImage || null;
  ...
  // Terms image
  const termsImgTag = termsImage
    ? `<img src="${termsImage.startsWith("data:") ? termsImage : `${BASE_URL}${termsImage}`}" style="margin-top:8px;max-width:100%;border-radius:4px;" />`
    : "";
  ...
  <div class="container">
    ...
    ${termsAndConditions ? `
      <div style="margin-bottom:16px;">
        ...
        <div style="font-size:10px;color:#4b5563;">${termsAndConditions}</div>
        ${termsImgTag}
      </div>
    ` : ''}
