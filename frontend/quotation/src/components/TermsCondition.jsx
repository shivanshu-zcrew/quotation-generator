...
export const newSection = () => ({
  ...
  heading: "",
  points: [newPoint()],
  images: [], // Array of images (base64 during editing, Cloudinary URLs after save)
});
...
// ─────────────────────────────────────────────────────────────
// sectionsToHTMLWithoutImages  — for DB storage (NO base64 images)
// ─────────────────────────────────────────────────────────────
export const sectionsToHTMLWithoutImages = (sections) => {
  ...
  return safeSections
    .map((sec) => {
      ...
      if (!sec.heading && !filledPts.length) return "";
      ...
      if (sec.heading) {
        html += `
          <div style="display:flex;align-items:baseline;gap:6px;margin-bottom:5px;">
            ...
            <span style="font-weight:700;font-size:11px;color:#0f172a;">${escapeHtml(sec.heading)}</span>
          </div>
        `;
      }
      ...
      filledPts.forEach((p, j) => {
        html += `
          <div style="display:flex;align-items:flex-start;gap:6px;margin-bottom:3px;padding-left:18px;">
            <span style="font-weight:600;color:#6366f1;font-size:10px;min-width:28px;flex-shrink:0;">${sNum}.${j + 1}</span>
            <span style="font-size:10px;color:#374151;line-height:1.5;">${escapeHtml(p.text)}</span>
          </div>
        `;
      });

      // NO IMAGES HERE - images will be stored separately in termsImages array
      html += "</div>";
      return html;
    })
    .join("");
...
// ─────────────────────────────────────────────────────────────
// sectionsToHTML  — for PDF generation (includes Cloudinary images)
// ─────────────────────────────────────────────────────────────
export const sectionsToHTML = (sections) => {
  ...
  return safeSections
    .map((sec) => {
      ...
      // Section images - using Cloudinary URLs (from DB after save)
      if (images.length > 0) {
        ...
        images.forEach((img) => {
          if (img && img.url && !img.url.startsWith('data:')) {
            // Only include Cloudinary URLs, not base64
            html += `
              <div style="position:relative;">
                <img src="${img.url}" style="max-width:200px;max-height:150px;border-radius:4px;border:1px solid #e2e8f0;" />
              </div>
            `;
          }
        });
...
// ─────────────────────────────────────────────────────────────
// In TermsCondition.jsx - update htmlToSections
export const htmlToSections = (html, cloudinaryImages = []) => {
  // If no HTML but we have cloudinary images, create sections for them
  if (!html || !html.trim()) {
    if (cloudinaryImages.length > 0) {
      // Put all images in the first section
      const images = cloudinaryImages.map((img, idx) => ({
  ...
  try {
    ...
    if (sectionDivs.length > 0) {
      return sectionDivs.map((sectionDiv, sectionIndex) => {
        ...
        // ✅ Get cloudinary images that belong to this section
        // You need to store sectionIndex when uploading, or put all in first section
        let sectionImages = [];
        
        if (cloudinaryImages.length > 0) {
          // If we have sectionIndex stored, filter by it
          const imagesWithIndex = cloudinaryImages.filter(img => img.sectionIndex === sectionIndex);
          if (imagesWithIndex.length > 0) {
            sectionImages = imagesWithIndex;
          } else if (sectionIndex === 0) {
            // For backward compatibility, put all images in first section
            sectionImages = cloudinaryImages;
          }
        }
    ...
    // If no sections found but we have cloudinary images, create a section
    if (cloudinaryImages.length > 0) {
      return [{
        id: `sec-${Date.now()}`,
        heading: "",
...
export default function TermsEditor({ sections = [], onChange }) {
  ...
  const updatePoint = (secId, ptId, text) => {
   ...
   const handleAddImage = (secId, e) => {
    ...
    reader.onload = () => {
      const sec = safeSections.find((s) => s && s.id === secId);
      if (sec) {
        const currentImages = Array.isArray(sec.images) ? sec.images : [];
        const newImage = {
        ...
        updateSection(secId, { images: [...currentImages, newImage] });
      }
    };
  ...
  const handleRemoveImage = (secId, imgId) => {
    ...
    if (sec) {
      const currentImages = Array.isArray(sec.images) ? sec.images : [];
      updateSection(secId, { 
        images: currentImages.filter((img) => img && img.id !== imgId) 
      });
    }
  };
      ...
      {safeSections.map((sec, secIdx) => {
        ...
        return (
          ...
          >
            ...
            {/* Actions - Add Point & Add Image (always visible) */}
            <div
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              ...
              <input
                ...
                id={`sec-img-${sec.id}`}
                style={{ display: "none" }}
                onChange={(e) => handleAddImage(sec.id, e)}
              />
              <label
                htmlFor={`sec-img-${sec.id}`}
                style={{
                  background: images.length > 0 ? "#fef3c7" : "#f0fdf4",
                  color: images.length > 0 ? "#92400e" : "#065f46",
                  border: "none",
                  borderRadius: "7px",
                  padding: "0.3rem 0.7rem",
                  fontSize: "0.78rem",
            ...
            {/* Images preview - display all images in a grid */}
            {images.length > 0 && (
              <div
                style={{
                  marginTop: "0.75rem",
              ...
              >
                {images.map((img, imgIdx) => (
                  ...
                  >
                    ...
                    />
                    <button
                      onClick={() => handleRemoveImage(sec.id, img.id)}
                      style={{
                        position: "absolute",
                        top: "-7px",
                        right: "-7px",
...
// ─────────────────────────────────────────────────────────────
// TermsViewer  — view mode (uses Cloudinary images from DB)
// ─────────────────────────────────────────────────────────────
        ...
        return (
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              {/* Images - display Cloudinary images in grid */}
              {images.length > 0 && (
                <div style={{ 
                  paddingLeft: "2rem", 
                  marginTop: "0.75rem",
